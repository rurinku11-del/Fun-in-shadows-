...
@bot.message_handler(func=lambda m: group(m) and bool(m.from_user),
 content_types=["text","photo","video","document","audio","voice","sticker","animation"])
def tracker(m): track(m)

@bot.message_handler(commands=["call"])
def call(m):
    if not group(m):return
    rows=[x for x in members(m.chat.id) if x[0]!=m.from_user.id]
    if not rows:return temp(m.chat.id,"📢 No active members found.")
    tags=[mention(x[0],x[1]) for x in rows]
    lines=[" ".join(tags[i:i+2]) for i in range(0,len(tags),2)]
    p=m.text.split(maxsplit=1); msg=p[1].strip() if len(p)>1 else "Kaha ho"
    temp(m.chat.id,"\n".join(lines)+f"\n\n              <b>{html.escape(msg)}</b> 👀")
        ...
        try:
            ...
            if t.hour==20 and t.minute==0:
                ...
                for chat in chats:
                    ...
                    if us:
                        w=random.choice(us);bot.send_message(chat,f"🎁🏆 Giveaway winner: {mention(w,'Winner')}!")
...
def helpcmd(m):bot.send_message(m.chat.id,HELP)
