...
def call(m):
    ...
    # Tag every known non-bot member in this group.
    # Telegram's Bot API does not provide a general "list all members"
    # endpoint, so the bot can only tag members it has previously seen.
    rows = [x for x in members(m.chat.id) if x[0] != m.from_user.id]

    if not rows:
        return temp(m.chat.id, "📢 No members have been seen by the bot yet.")

    tags = [mention(x[0], x[1]) for x in rows]

    # Exactly 4 members per line.
    lines = [" ".join(tags[i:i+4]) for i in range(0, len(tags), 4)]

    parts = m.text.split(maxsplit=1)
    msg = parts[1].strip() if len(parts) > 1 else "Kaha ho"
