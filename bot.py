...
music_calls = PyTgCalls(music_mt) if (music_mt and PyTgCalls) else None
...
async def start(update, context):
    ...
    text = (
        ...
        "/game — play one of 3 games\n"
        "/rank — view coin rankings\n"
        "/participate — enter the 8 PM IST giveaway\n"
        "/activity — view group activity\n"
        "/fight — start a 2-coin quiz battle\n"
        "/groupinfo — group info (1 coin)\n"
        
        
        
        "/userinfo — user info (2 coins)\n"
        "/give_coin <amount> — give coins to a replied user\n"
...
async def help_cmd(update, context):
    text = "For any problem or any changes want dm bot owner @Shadow_atomic_21"
    msg = await update.message.reply_text(text, parse_mode=ParseMode.HTML)
    schedule_delete(context, msg)
...
async def calladmins_cmd(update, context):
    # Anyone in the group can use this command.
