...
async def voice_note_reply(update, context):
    if not update.message or not update.message.voice:
        return
    text = "🎙️ Voice note mil gaya bhai 😎 Main sun raha hoon."
    if edge_tts:
        try:
            path = f"/tmp/fis_voice_{update.message.message_id}.mp3"
...
async def volume_cmd(update, context):
    ...
    if volume < 0:
        ...
        return
    try:
        participant = await music_calls.mtproto_client.resolve_peer(await music_calls.mtproto_client.get_id())
        await music_calls.mtproto_client.change_volume(update.effective_chat.id, volume, participant)
...
async def start(update, context):
    ...
    text = (
        ...
        "/health — user ki activity health (1 🪙)\n"
...
async def help_cmd(update, context):
    text = (
        ...
        "Group: /health (1 🪙), /activity, /groupinfo (1 🪙), /userinfo (2 🪙)\n"
...
async def health_cmd(update, context):
    ...
    if update.effective_chat.type not in ("group", "supergroup"):
        ...
        return
    if not spend_coins(chat_id, update.effective_user.id, 1):
        msg = await update.message.reply_text("🪙 Health check ke liye 1 coin chahiye.")
        schedule_delete(context, msg)
        return
...
def main():
    ...
    app.add_handler(
        MessageHandler(filters.ChatType.PRIVATE & ~filters.COMMAND & ~filters.AUDIO, private_chat)
    )
