import os, sqlite3, random, threading, time, html, asyncio

# Pyrogram 2.0 uses asyncio.get_event_loop() during import.
# Python 3.11+ may not create a loop automatically.
try:
...
def call(m):
    ...
    if not rows:
        # If the user session cannot see this group, do not pretend that the
        # database contains every member. It only contains members seen by bot.
        fallback = members(m.chat.id)
        if fallback:
            rows = fallback
        else:
            return temp(m.chat.id, "📢 Full member list nahi mil rahi. Check karo ki SESSION_STRING wala Telegram account group mein added hai.")

    p = m.text.split(maxsplit=1)
    msg = p[1].strip() if len(p) > 1 else "Kaha ho"

    tags = [mention(uid, name) for uid, name, username in rows]

    # Telegram message text has a practical size limit. Split tags into
    # several messages so large groups are handled instead of failing.
    chunks = []
    current = []
    current_len = 0
    for tag in tags:
        extra = len(tag) + (1 if current else 0)
        if current and current_len + extra > 3500:
            chunks.append(" ".join(current))
            current = []
            current_len = 0
        current.append(tag)
        current_len += len(tag) + (1 if current_len else 0)
    if current:
        chunks.append(" ".join(current))

    for index, chunk in enumerate(chunks):
        suffix = f"\n\n<b>{html.escape(msg)}</b> 👀" if index == len(chunks) - 1 else ""
...
def giveaway_worker():
    while True:
        try:
            ...
            print("Giveaway worker error:", repr(e))
        time.sleep(30)
...
def auto_coins():
    while True:
        ...
        try:
            ...
            print("Auto coin worker error:", repr(e))
...
if __name__=="__main__":
    ...
    try:
        ...
        print("Telegram user session could not start:", repr(e))
    ...
    while True:
        try:bot.infinity_polling(timeout=30,long_polling_timeout=30,skip_pending=True)
            ...
            print("Bot polling error:", repr(e))
            time.sleep(5)
